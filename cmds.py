...
class Commands:
    ...
    def ban(self,user,cmd,args):
        messages.whiteMessage(client.dumps((args[0],"The ban hammer has spoken!",True),methodname="BanAndBlackList").encode(),
    ...
    def unban(self,user,cmd,args):
        messages.whiteMessage(client.dumps((args[0],),methodname="UnBan").encode(),user,
                              MessageGenerators.getLevel(self.config["WhiteList"][user]),MessageGenerators.getLevel("Operator"))
        messages.whiteMessage(client.dumps((args[0],),methodname="UnBlackList").encode(),user,
                              MessageGenerators.getLevel(self.config["WhiteList"][user]),MessageGenerators.getLevel("Operator"))
    def kick(self,user,cmd,args):
        messages.whiteMessage(client.dumps((args[0],"You've been booted, get out!"),methodname="Kick").encode(),
                              user,MessageGenerators.getLevel(self.config["WhiteList"][user]),MessageGenerators.getLevel("Moderator"))
    def mute(self,user,cmd,args):
        messages.whiteMessage(client.dumps((args[0],),methodname="Ignore").encode(),user,
                              MessageGenerators.getLevel(self.config["WhiteList"][user]),MessageGenerators.getLevel("Moderator"))
    def unmute(self,user,cmd,args):
        messages.whiteMessage(client.dumps((args[0],),methodname="UnIgnore").encode(),user,
                              MessageGenerators.getLevel(self.config["WhiteList"][user]),MessageGenerators.getLevel("Moderator"))
    def forceskip(self,user,cmd,args):
        messages.whiteMessage(client.dumps(tuple(),methodname="NextChallenge").encode(),user,
                              MessageGenerators.getLevel(self.config["WhiteList"][user]),MessageGenerators.getLevel("Moderator"))
    def forcerestart(self,user,cmd,args):
        messages.whiteMessage(client.dumps(tuple(),methodname="RestartChallenge").encode(),user,
                              MessageGenerators.getLevel(self.config["WhiteList"][user]),MessageGenerators.getLevel("Moderator"))
    def delchall(self,user,cmd,args):
        messages.whiteMessage(client.dumps((args[0],),methodname="RemoveChallenge").encode(),user,
                              MessageGenerators.getLevel(self.config["WhiteList"][user]),MessageGenerators.getLevel("Operator"))
    def updchalls(self,user,cmd,args):
        try:
            if user in self.whitelist.keys():
                if MessageGenerators.getLevel(self.config["WhiteList"][user]) >= MessageGenerators.getLevel("Operator"):
